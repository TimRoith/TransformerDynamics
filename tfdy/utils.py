...
def sph2cart(phi):
    return torch.stack([torch.sin(phi[..., 1]) * torch.cos(phi[..., 0]), 
                        torch.sin(phi[..., 1]) * torch.sin(phi[..., 0]),
                        torch.cos(phi[..., 1])], dim=-1)
...
def cart2sph(x):
    return torch.stack([
        torch.acos(x[...,2]),
        #torch.sign(x[..., 1]) * torch.acos(x[...,0]/torch.sqrt(x[..., 0]**2+x[..., 1]**2)) + (x[..., 1] < 0)*2*torch.pi,
